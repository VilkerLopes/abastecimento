document.addEventListener('DOMContentLoaded', function() {
    function carregarPassageiro() {
        ...
        .then(data => {
            const select = document.getElementById('passageiro');
            select.innerHTML = '';  // Limpa as opções anteriores

            if (data.length === 0) {
                select.innerHTML = '<option>Nenhum dado encontrado.</option>';
            } else {
                select.innerHTML = '<option value="">Selecione um passageiro</option>';  // Adiciona uma opção padrão
